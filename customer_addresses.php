<?php 
    ...
    try {
        $stmt_del = $obj->con1->prepare("DELETE FROM `customer_addresses` WHERE id=?");
...
<div class='p-6 animate__animated' x-data='pagination'>
    <h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">Customer Addresses</h1>
    <div class="panel mt-6 flex items-center  justify-between relative">
        ...
        <button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:insertdata()">
            <i class="ri-add-line mr-1"></i> Add Customer  Addresses</button>
...
function insertdata(id) {
    eraseCookie("edit_id");
    eraseCookie("view_id");
    window.location = "add_customer_addresses.php";
}
...
async function showAlert(id) {
    new window.Swal({
        ...
        if (result.isConfirmed) {
            var loc = "customer_addresses.php?flg=del&id=" + id ;
            window.location = loc;
        }
    });
}
