<?php 
    ...
    try {
        $stmt_del = $obj->con1->prepare("DELETE FROM `promocode` WHERE promo_id=?");
...
<div class='p-6 animate__animated' x-data='pagination'>
    <h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">Promocode</h1>
    <div class="panel mt-6 flex items-center  justify-between relative">
        ...
        <button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:insertdata()">
            <i class="ri-add-line mr-1"></i> Add Promocode</button>
    ...
    Alpine.data('pagination', () => ({
        ...
        init() {
            this.datatable = new simpleDatatables.DataTable('#myTable', {
                data: {
                    headings: ['Sr.No.', 'Name','Promocode',
                        'Discount', 'Max-Discount','Min-Amount','Start Date','End Date','Status','Action'
                    ],
                    data: [
                        <?php
                                $stmt = $obj->con1->prepare("SELECT * FROM `promocode` order by promo_id");
                                $stmt->execute();
                            ...
                            '<?php echo addslashes ($row["promocode"]); ?>',
                            '<?php echo addslashes ($row["discount"]); ?>',
                            '<?php echo addslashes($row["max_discount"]);?>',
                            '<?php echo addslashes($row["min_amount"]);?>',
                            '<?php echo addslashes($row["start_date"]);?>',
                            '<?php echo addslashes($row["end_date"]);?>',
                            ...
                            getActions(<?php echo $row["promo_id"];?>)
                        ],
                        <?php $i++;}
                                    ?>
                    ],
...
function insertdata(id) {
    eraseCookie("edit_id");
    eraseCookie("view_id");
    window.location = "add_promocode.php";
}
