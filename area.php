...
if (isset($_REQUEST["flg"]) && $_REQUEST["flg"] == "del") {
    try {
        $stmt_del = $obj->con1->prepare(
            "delete from area where srno='" . $_REQUEST["srno"] . "'"
        );
        $Resp = $stmt_del->execute();
        if (!$Resp) {
            if (strtok($obj->con1->error, ":") == "Cannot delete or update a parent row") {
                throw new Exception("Area is already in use!");
            }
        }
        $stmt_del->close();
    } catch (\Exception $e) {
...
<div class='p-6 animate__animated' x-data='pagination'>
	<h1 class="dark:text-white-dar  pb-8 text-3xl font-bold">Area</h1>
	<div class="panel mt-6 flex items-center  justify-between relative">
		...
		<button type="button" class="p-2 btn btn-primary m-1 add-btn" onclick="javascript:add_data()">
			<i class="ri-add-line mr-1"></i> Add-Area </button>
...
async function showAlert(id) {
   new window.Swal({
    ...
    if (result.isConfirmed) {
     var loc = "area.php?flg=del&srno=" +id;
     window.location = loc;
 }
});
}
